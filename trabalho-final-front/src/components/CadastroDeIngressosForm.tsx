...
const schema = z.object({
  ...
  assento: z
  .string()
  .refine((value) => /^[0-9]+$/.test(value), { message: "O número do assento deve ser um número válido." })
  .refine((value) => Number(value) >= 0, { message: "O número do assento deve ser um número válido." }),
  preco: z
  .string()
...
const CadastroDeIngressosForm = () => {
  ...
  const onSubmit = ({
    ...
    const ingresso: Ingresso = {
      ...
      sessao: { id: sessao, tituloFilme: "", horaInicio: "" , produto: sessao.produto},
    };
    reset();
  ...
  useEffect(() => {
    ...
    if (ingressoSelecionado?.codIngresso) {
      reset();
      setValue("assento", String(ingressoSelecionado.assento));
  ...
  <form onSubmit={handleSubmit(onSubmit)}>
    <div className="row mb-2">
      <div className="col-xl-2 fw-bold text-start align-self-center">Assento</div>
      <div className="col-xl-10">
        ...
        />
        <div className="invalid-feedback">{errors.assento?.message}</div>
      </div>
    </div>
